...
class ClassVisitorTest {
    ...
    @Test
    void testSample01ClassParsing() throws Exception {
    ...
    @Test
    void testSample02ClassParsing() throws Exception {
        File file = new File("src/test/java/io/github/opsord/architecture_evaluator_backend/test_codes/Sample02.java");
        String code = Files.readString(file.toPath());
        JavaParser javaParser = new JavaParser();
        CompilationUnit cu = javaParser.parse(code).getResult().orElseThrow(() -> new RuntimeException("Parsing failed"));
        ...
        assertEquals(1, classes.size());
        ClassInstance instance = classes.get(0);
        assertEquals("Sample02", instance.getName());
        assertEquals(2, instance.getMethods().size()); // foo + main
    }

    @Test
    void testSample03ClassParsing() throws Exception {
        File file = new File("src/test/java/io/github/opsord/architecture_evaluator_backend/test_codes/Sample03.java");
        String code = Files.readString(file.toPath());
        JavaParser javaParser = new JavaParser();
        CompilationUnit cu = javaParser.parse(code).getResult().orElseThrow(() -> new RuntimeException("Parsing failed"));
        ...
        assertEquals(1, classes.size());
        ClassInstance instance = classes.get(0);
        assertEquals("Sample03", instance.getName());
        assertEquals(2, instance.getMethods().size()); // main + CheckAccess
    }
}