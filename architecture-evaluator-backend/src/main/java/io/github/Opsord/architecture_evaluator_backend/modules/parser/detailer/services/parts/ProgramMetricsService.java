...
public class ProgramMetricsService {
    ...
    public ProgramMetricsDTO generateProgramMetrics(CustomCompilationUnitDTO customCompilationUnit) {
        ...
        for (MethodDTO method : customCompilationUnit.getMethods()) {
            ...
            // Sum the approxMcCabeCC for each method
            approxMcCabeCC += calculateApproxMcCabeCC(method);
        }

        double improvedCC = calculateImprovedCC(
                numberOfMethods,
                sumOfExecutableStatements,
                maxInputParameters,
                maxOutputParameters,
                totalLinesOfCode);
        ...
        programMetrics.setApproxMcCabeCC(approxMcCabeCC);
        programMetrics.setImprovedCC(improvedCC);
